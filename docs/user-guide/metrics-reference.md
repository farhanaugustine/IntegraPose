...
- 0 deg = facing up in the video frame
- 90 deg = facing right
- 180 deg = facing down
- 270 deg = facing left
...
- **Speed (px/frame)**: `v_t = ||p_t - p_{t-1}|| / delta_t`, where `delta_t = 1` frame if FPS is unknown. Biological readout: overall locomotor intensity.
- **Acceleration (px/frame^2)**: `a_t = (v_t - v_{t-1}) / delta_t`. Biological readout: startle bursts vs. cruising.
...
- **Angular velocity (deg/frame)**: `omega_t = |orientation_t - orientation_{t-1}|` (smallest circular difference). Biological readout: turning rate; spikes mark re-aiming events.
