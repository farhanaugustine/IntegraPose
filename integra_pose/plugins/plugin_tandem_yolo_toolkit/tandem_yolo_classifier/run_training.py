...
from integra_pose.utils.safe_model_io import save_normalization_stats
...
def run_training_pipeline(config: Dict):
    ...
    try:
        ...
        norm_stats_path = os.path.join(data_cfg['output_directory'], 'norm_stats.npz')
        save_normalization_stats(norm_stats_path, mean=mean, std=std)
